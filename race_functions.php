...
function race_open_check($race)
{
	...
	while($row=mysqli_fetch_assoc($result))
	{
		...
		if($row['race_date'] == $day)
		{
			...
			else if($diff < -1 and table_update($race))
			{
				$flag = 1;
				//echo "in else if <br>";
			}
...
function race_close()
{
	...
	while($row=mysqli_fetch_assoc($result))
	{
		if(race_open_check($row['race_name'])== 1)
		{
			...
			$myfile=fopen("/home/varun/last_race.txt","w");
...
{
	$myfile=fopen("/home/varun/last_race.txt","r");
	$var = fread($myfile,20);
	//$name = fgets($myfile);
	fclose($myfile);
	return $var;
...
function racename_to_raceid($racename)
{
	$dbhost = 'localhost';
	$db1='club';
	$db2='derby';
	...
	{
		$sql = "SELECT * FROM club.racing_history WHERE race_name ='".$racename."';";
		$result = mysqli_query($conn1,$sql);
...
function get_first($racename)
{
	$race_id=racename_to_raceid($racename);
	$dbhost = 'localhost';
	$db1='club';
	$db2='derby';
	$dbuser = 'admin';
